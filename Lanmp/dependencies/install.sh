...
if [ "$ifredhat" != "" ];then
  ...
  apt-get -y install make cmake gcc g++ unzip build-essential libncurses5-dev libfreetype6-dev libxml2 libxml2-dev libssl-dev libcurl4-openssl-dev pkg-config libjpeg62-dev libpng-dev libfreetype6-dev libsasl2-dev libpcre3-dev autoconf libperl-dev libtool libaio*
  iptables -F
elif [ "$ifdebian" != "" ];then
  apt-get -y update
  \mv /etc/apache2 /etc/apache2.bak &> /dev/null
