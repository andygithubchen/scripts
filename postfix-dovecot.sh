...
# Edit here ..
mysqlPass='QsY3FdpjwyjWzFv1z4b8vlM69NAXwEv9'     # mysql root password here

database="email"      # name database email
dbHost="127.0.0.1"             # user database email
dbUser="email"             # user database email
dbPort="3306"              # database port
dbUserPass="KNXUsbsggFHQDRjo"    # password user database email

Domain="waxxoo.com"         # your Domain
Email="admin@waxxoo.com"    # Email with your domain
EmailPass="135246kjs"   # password email
#
...
createDB(){
    ...
    GRANT SELECT ON $database.* TO '$dbUser'@'${dbHost}' IDENTIFIED BY '$dbUserPass';
    FLUSH PRIVILEGES;
    USE $database;
...
sudo sed -i 's/#connect =/connect = host='$dbHost' dbname='$database' port='$dbPort' user='$dbUser' password='$dbUserPass'/' /etc/dovecot/dovecot-sql.conf.ext
...
#Error reading configuration: Invalid settings: postmaster_address setting not given lda: Fatal: Internal error occurred. Refer to server log for more information. 4.2.1 Message can not be delivered at this time
# echo "postmaster_address = admin@example.com" >> /etc/dovecot/conf.d/15-lda.conf
# doveadm reload
